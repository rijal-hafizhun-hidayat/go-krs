                                ...
                                <div class="p-5">
                                    ...
                                    <?php
                                        if (isset($_POST['login'])) {
                                            $user = $_POST['username'];
                                            $pass = $_POST['password'];
                                            $cek = mysqli_query($konek, "SELECT mahasiswa.nim, mahasiswa.password, mahasiswa.nama_mahasiswa FROM mahasiswa WHERE nim = '$user' AND password = '$pass'");
                                            $baca = mysqli_fetch_array($cek);
                                            
                                            $username = $baca['nim'];
                                            $password = $baca['password'];
                                            $nama = $baca['nama_mahasiswa'];
                                            if ($user == $username && $pass == $password) {
                                                $_SESSION['nama'] = $nama;
                                                $_SESSION['nim'] = $username;
                                                header('location:dashbord.php?status=sukses');
                                            }else{?>
